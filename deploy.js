...
const config = {
  channelAccessToken:
    "ImqXwkfn8xKOq9R9wj7yCaPBIzK3SR9+mKNUH4FDI35XMqeQeD9hpZD6tg0IthitOITqrjTnst/J1yglV6J4WTeMxmHyQck9bzeOJ8+dXUmQ8jCYec8S8kaO/nniwQNQhmh57cHlxfyeHxpoXOVGLAdB04t89/1O/w1cDnyilFU=",
  channelSecret: "2cbe3d3fb6620db86568f0b431710e83",
};
const apiKey = `eb81914dfe2d495f868cd881317aeb4d`;
  ...
  const getTopNews = async () => {
    try {
      const response = await axios.get(
        `https://newsapi.org/v2/top-headlines?country=tw&apiKey=${apiKey}`
      );
  ...
  const pushTopNews = async () => {
    ...
    if (topNews) {
          ...
          contents: {
            ...
            body: {
              ...
              contents: [
                ...
                {
                  type: "text",
                  text: topNews[0].description,
                  size: "sm",
                  wrap: true,
                },
                {
                  type: "text",
                  text: topNews[0].source.name,
                  size: "sm",
                  wrap: true,
                },
                {
            ...
            footer: {
              type: "box",
              layout: "horizontal",
              contents: topNews.slice(1, 10).map((article) => ({
                type: "button",
                action: {
                  type: "uri",
      ...
      try {
        const message = {
          type: "text",
          text: "Here are the top news in Taiwan!",
        };
  ...
  function autoWeather() {
    cron.schedule("0 0 * * *", () => {
      getTaiwanWeather().then((weatherData) => {
        ...
        let push = client.pushMessage(userId, { type: "text", text: message });
        push.destroy();
      });
    });
  }
